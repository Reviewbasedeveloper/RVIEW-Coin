...
*Setup instructions for a gitian build of ReviewBase_Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ReviewBase_Coin
...
- [Building ReviewBase_Coin](#building-reviewbase_coin)
...
Clone the git repositories for reviewbase_coin and gitian and then checkout the reviewbase_coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/reviewbase_coin-crypto/reviewbase_coin.git
cd reviewbase_coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the reviewbase_coin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building ReviewBase_Coin
----------------

To build ReviewBase_Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the reviewbase_coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/reviewbase_coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/reviewbase_coin-crypto/reviewbase_coin
...
For example:
```bash
URL=https://github.com/crowning-/reviewbase_coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit reviewbase_coin=${COMMIT} --url reviewbase_coin=${URL} ../reviewbase_coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit reviewbase_coin=${COMMIT} --url reviewbase_coin=${URL} ../reviewbase_coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit reviewbase_coin=${COMMIT} --url reviewbase_coin=${URL} ../reviewbase_coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/reviewbase_coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/reviewbase_coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/reviewbase_coin-build.assert
```
...
[reviewbase_coin/gitian.sigs](https://github.com/reviewbase_coin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
