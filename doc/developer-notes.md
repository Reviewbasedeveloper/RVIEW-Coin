...
- [Development guidelines](#development-guidelines)
    - [General ReviewBase_Coin](#general-reviewbase_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ReviewBase_Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play RVIEWs (tRVIEW)" on the test network, if you
...
ReviewBase_Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/reviewbase_coin-project/reviewbase_coin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_reviewbase_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_reviewbase_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/reviewbase_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_reviewbase_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates RVIEWs (if wallet is enabled).
...
However, in open source software such as ReviewBase_Coin, where everyone uses
...
pay attention to for reviewers of ReviewBase_Coin code.

General ReviewBase_Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ReviewBase_Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ReviewBase_Coin so that they can be integrated
...
file descriptors it will cause problems with ReviewBase_Coin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:ReviewBase_Coin-Project/ReviewBase_Coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `reviewbase_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `reviewbase_coin-cli` and the GUI debug console use this table to determine how to
