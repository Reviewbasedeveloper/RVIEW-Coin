...
#define URI_SCHEME "reviewbase_coin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ReviewBase Coin address (e.g. %1)").arg("R4EFR83SQgA1zrW16hjcHJtcfip5krt1sD"));
...
{
    // return if URI is not valid or is no ReviewBase_Coin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::RVIEW, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert reviewbase_coin:// to reviewbase_coin:
    //
    //    Cannot handle this later, because reviewbase_coin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::RVIEW, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    ...
    /* Open reviewbase_coin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ReviewBase_Coin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ReviewBase_Coin.lnk
...
{
    return GetAutostartDir() / "reviewbase_coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a reviewbase_coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ReviewBase_Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the reviewbase_coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add reviewbase_coin app to startup item list
