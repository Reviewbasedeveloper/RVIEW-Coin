...
#include "qt/reviewbase_coin/forms/ui_masternodeswidget.h"
#include "qt/reviewbase_coin/qtutils.h"
#include "qt/reviewbase_coin/mnrow.h"
#include "qt/reviewbase_coin/mninfodialog.h"

#include "qt/reviewbase_coin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(ReviewBase_CoinGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN){
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the reviewbase_coin.conf file\nof your remote server and start it, "
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:11915 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * 2000)){
            inform(tr("No enough balance to create a master node, 2,000 RVIEW required."));
            return;
        }
        showHideOp(true);
