...
#include "qt/reviewbase_coin/forms/ui_addresseswidget.h"
...
AddressesWidget::AddressesWidget(ReviewBase_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a ReviewBase Coin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress rviewAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(rviewAdd.Get(), label.toUtf8().constData(), "send")) {
