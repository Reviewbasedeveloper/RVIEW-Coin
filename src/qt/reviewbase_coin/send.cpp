...
#include "qt/reviewbase_coin/send.h"
#include "qt/reviewbase_coin/forms/ui_send.h"
#include "qt/reviewbase_coin/addnewcontactdialog.h"
#include "qt/reviewbase_coin/qtutils.h"
#include "qt/reviewbase_coin/sendchangeaddressdialog.h"
#include "qt/reviewbase_coin/optionbutton.h"
#include "qt/reviewbase_coin/sendconfirmdialog.h"
#include "qt/reviewbase_coin/myaddressrow.h"
...
SendWidget::SendWidget(ReviewBase_CoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (RVIEW)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a ReviewBase Coin address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 RVIEW");
	setCssProperty(ui->labelAmountSend, "text-body1");

	//send button
	ui->pushButtonSave->setText(tr("Send RVIEW"));
	...
	setCssProperty(coinIcon, "coin-icon-rview");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_RVIEW, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any RVIEW to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress rviewAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(rviewAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(rviewAdd.Get(), dialog->getLabel().toStdString(), "send")) {
