...
#include "qt/reviewbase_coin/dashboardwidget.h"
#include "qt/reviewbase_coin/forms/ui_dashboardwidget.h"
#include "qt/reviewbase_coin/sendconfirmdialog.h"
#include "qt/reviewbase_coin/txrow.h"
...
DashboardWidget::DashboardWidget(ReviewBase_CoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	// Staking Information
	ui->labelMessage->setText(tr("Amount of RVIEW staked."));
	setCssSubtitleScreen(ui->labelMessage);
	setCssProperty(ui->labelSquareRview, "square-chart-rview");
	setCssProperty(ui->labelRview, "text-chart-rview");
	...
	ui->labelAmountRview->setText("0 RVIEW");
	setCssProperty(ui->labelAmountRview, "text-stake-rview-disable");
	...
	ui->btnHowTo->setText(tr("How to get RVIEW"));
...
ChartData DashboardWidget::loadChartData(bool withMonthNames) {
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData.amountsByCache.contains(num)) {
			...
			rview = (pair.first != 0) ? pair.first / 100000000 : 0;

			chartData.totalRview += pair.first;

		}

		chartData.xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData.valuesRview.append(rview);


		int max = std::max(rview, rview);
...
void DashboardWidget::onChartRefreshed() {
	...
	set0->append(chartData.valuesRview);


	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData.totalRview > 0) {
		setCssProperty(ui->labelAmountRview, "text-stake-rview");
	}
	else {
		setCssProperty(ui->labelAmountRview, "text-stake-rview-disable");
	}
	forceUpdateStyle({ ui->labelAmountRview });
	ui->labelAmountRview->setText(GUIUtil::formatBalance(chartData.totalRview, nDisplayUnit));
