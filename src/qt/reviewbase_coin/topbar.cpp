...
#include "qt/reviewbase_coin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/reviewbase_coin/lockunlock.h"
#include "qt/reviewbase_coin/qtutils.h"
#include "qt/reviewbase_coin/receivedialog.h"
...
TopBar::TopBar(ReviewBase_CoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopRview}, "amount-small-topbar");
    setCssProperty({ui->labelAmountRview}, "amount-topbar");
    setCssProperty({ui->labelPendingRview, ui->labelImmatureRview}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("ReviewBase_Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // Set
    QString totalRview = GUIUtil::formatBalance(rviewAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopRview->setText(totalRview);
    

    // Expanded
    ui->labelAmountRview->setText(totalRview);
    

    ui->labelPendingRview->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureRview->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
