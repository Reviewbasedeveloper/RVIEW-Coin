...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::RVIEW);
    ...
    if (settings.contains("nAnonymizeReviewbase_CoinAmount") || reset)
        SoftSetArg("-anonymizereviewbase_coinamount", settings.value("nAnonymizeReviewbase_CoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in reviewbase_coin.cpp during shotdown to also remove the window positions
