                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select RVIEW Denominations to Spend");
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::RVIEW;
    ...
    QString toolTip4 = tr("Can vary +/- %1 urview per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show ReviewBase_Coin address. In tree mode, address is not shown again for direct wallet address outputs
