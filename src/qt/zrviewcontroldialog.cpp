...
#include "ui_zrviewcontroldialog.h"
...
std::set<std::string> ZRviewControlDialog::setSelectedMints;
std::set<CMintMeta> ZRviewControlDialog::setMints;

bool CZRviewControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRviewControlDialog::COLUMN_DENOMINATION || column == ZRviewControlDialog::COLUMN_VERSION || column == ZRviewControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRviewControlDialog::ZRviewControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRviewControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zRVIEW Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZRview->setProperty("cssClass", "text-main-purple");
    ui->labelZRview_int->setProperty("cssClass", "text-main-purple");
...
ZRviewControlDialog::~ZRviewControlDialog()
{
    delete ui;
}

void ZRviewControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRviewControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRviewControlWidgetItem* itemDenom(new CZRviewControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRviewControlWidgetItem *itemMint = new CZRviewControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zrviewTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zrviewTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRVIEW.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRVIEW seed used to mint this zRVIEW is not the same as currently hold in the wallet";
            else
...
void ZRviewControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRviewControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRview_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZRviewControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRviewControlDialog::GetSelectedMints()
{
...
void ZRviewControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
