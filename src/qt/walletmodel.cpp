...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered reviewbase_coin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 RVIEW.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal reviewbase_coin:URI (reviewbase_coin:XyZ...?message=example)
            {
...
static void NotifyzRVIEWReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRVIEWReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRVIEWReset.connect(boost::bind(NotifyzRVIEWReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRVIEWReset.disconnect(boost::bind(NotifyzRVIEWReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zrviewTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
