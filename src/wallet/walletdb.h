...
public:
    ...
    bool WriteZRVIEWSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZRVIEWSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZRVIEWSeed_deprecated(uint256& seed);
    bool EraseZRVIEWSeed();
    bool EraseZRVIEWSeed_deprecated();

    bool WriteZRVIEWCount(const uint32_t& nCount);
    bool ReadZRVIEWCount(uint32_t& nCount);
