...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zrviewspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 RVIEW at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRVIEW send
enum ZerocoinSpendStatus {
    ZRVIEW_SPEND_OKAY = 0,                            // No error
    ZRVIEW_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRVIEW_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRVIEW_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRVIEW_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRVIEW_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRVIEW_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRVIEW_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRVIEW_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRVIEW_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRVIEW_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRVIEW_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRVIEW_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRVIEW_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRVIEW_SPENT_USED_ZRVIEW = 14,                      // Coin has already been spend
    ZRVIEW_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRVIEW_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    std::string GetUniqueWalletBackupName(bool fzrviewAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzRVIEWTracker> zrviewTracker;
    ...
    void setZWallet(CzRVIEWWallet* zwallet)
    {
        zwalletMain = zwallet;
        zrviewTracker = std::unique_ptr<CzRVIEWTracker>(new CzRVIEWTracker(strWalletFile));
    }

    CzRVIEWWallet* getZWallet() { return zwalletMain; }
    ...
    /** zRVIEW reset */
    boost::signals2::signal<void()> NotifyzRVIEWReset;
