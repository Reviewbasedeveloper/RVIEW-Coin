...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzRVIEWWallet *czRVIEWWallet = new CzRVIEWWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czRVIEWWallet->GenerateDeterministicZRVIEW(denom, coin, dMint, true);
        czRVIEWWallet->UpdateCount();
