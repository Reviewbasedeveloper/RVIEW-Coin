...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zrviewObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zrviewObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zrviewObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zRVIEWsupply", zrviewObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZRVIEW()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zRVIEWsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zRVIEW denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zRVIEW denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zRVIEW denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zRVIEW denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zRVIEW denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zRVIEW denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zRVIEW denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zRVIEW denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zRVIEW denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in reviewbase coin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of reviewbase coin addresses\n"
            "     \"reviewbase coinaddress\"   	 	(string) reviewbase_coin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (txin.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!ZRVIEWModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zRVIEW excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zRVIEW mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zRVIEW mints included) over block range\n"
