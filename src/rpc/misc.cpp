...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total reviewbase coin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in reviewbase_coin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in reviewbase_coin/kb\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"reviewbase_coinaddress\"\n"
            "\nReturn information about the given reviewbase_coin address.\n"

            "\nArguments:\n"
            "1. \"reviewbase_coinaddress\"     (string, required) The reviewbase_coin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"reviewbase_coinaddress\", (string) The reviewbase_coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ReviewBase_Coin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are reviewbase_coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) reviewbase_coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"reviewbase_coinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"reviewbase_coinaddress\"  (string, required) The reviewbase_coin address to use for the signature.\n"
