...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zrviews that have been in mempool long
                    //and higher priority to zrviews that are large in value
                    ...
                    // zRVIEW spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zRVIEW spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zRVIEW serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                //Sign block with the zRVIEW key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zRVIEW key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ReviewBase CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zrviewTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zrviewTracker->ListMints(true, true, true); //update the state
        }
        return error("ReviewBase CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ReviewBase CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reviewbase_coin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ReviewBase CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ReviewBase CoinMiner exception");
    } catch (...) {
        LogPrintf("ReviewBase CoinMiner exception");
    }

    LogPrintf("ReviewBase CoinMiner exiting\n");
}
