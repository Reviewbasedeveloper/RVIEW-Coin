...
CzRVIEWTracker::CzRVIEWTracker(std::string strWalletFile)
{
...
CzRVIEWTracker::~CzRVIEWTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRVIEWTracker::Init()
{
...
bool CzRVIEWTracker::Archive(CMintMeta& meta)
{
...
bool CzRVIEWTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRVIEWTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRVIEWTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRVIEWTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRVIEWTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzRVIEWTracker::GetSerialHashes()
{
...
CAmount CzRVIEWTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRVIEWTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

//Does a mint in the tracker have this txid
bool CzRVIEWTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRVIEWTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRVIEWTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRVIEWTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRVIEWTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRVIEWTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRVIEWTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRVIEWTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRVIEWWallet* zRVIEWWallet)
{
    bool iszRVIEWWalletInitialized = (NULL != zRVIEWWallet);
    CMintMeta meta;
    ...
    if (! iszRVIEWWalletInitialized)
        zRVIEWWallet = new CzRVIEWWallet(strWalletFile);
    meta.isSeedCorrect = zRVIEWWallet->CheckSeed(dMint);
    if (! iszRVIEWWalletInitialized)
        delete zRVIEWWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRVIEWTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRVIEWTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRVIEWTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRVIEWTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRVIEWTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRVIEWTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRVIEWWallet* zRVIEWWallet = new CzRVIEWWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRVIEWWallet);
        }
        delete zRVIEWWallet;
        LogPrint("zero", "%s: added %d dzrview from DB\n", __func__, listDeterministicDB.size());
    }
