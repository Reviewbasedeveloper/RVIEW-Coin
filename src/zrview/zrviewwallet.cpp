...
CzRVIEWWallet::CzRVIEWWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrview seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRVIEWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRVIEWSeed_deprecated()) {
                    LogPrintf("%s: Updated zRVIEW seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrview seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrview wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRVIEWWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRVIEWCount(nCountLastUsed);
    else if (!walletdb.ReadZRVIEWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRVIEWWallet::Lock()
{
    seedMaster = 0;
}

void CzRVIEWWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRVIEWWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRVIEW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRVIEWWallet::LoadMintPoolFromDB()
{
...
void CzRVIEWWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRVIEWWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRVIEWWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrviewTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRVIEWWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRVIEW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrviewTracker which also adds to database
    pwalletMain->zrviewTracker->Add(dMint, true);
...
void CzRVIEWWallet::SeedToZRVIEW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRVIEWWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRVIEWWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZRVIEWCount(nCountLastUsed);
...
void CzRVIEWWallet::GenerateDeterministicZRVIEW(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRVIEWWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRVIEWWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRVIEWWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
